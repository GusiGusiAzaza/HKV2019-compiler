...
#define MFST_DIAGN_MAXSIZE 2 * ERROR_MESSAGE_MAXSIZE
...
#define MFST_TRACE6(LOG,c,k) *log.stream<<setw(4)<<left<< ++FST_TRACE_n<<": "<<left<<(c)<<(k) << endl;
...
namespace MFST
{
	...
	struct Mfst //магазинный автомат	
	{
		...
		{
			NS_OK,
			//найдено правило и цепочка, цепочка записана в стек
			NS_NORULE,
			//не найдено правила грамматики (ошибки в грамматике)
			NS_NORULECHAIN,
			//не найдена подходящая цепочка правила (ошибка в исходном коде)
			NS_ERROR,
			//неизвестный нетерминальный символ грамматики
			TS_OK,
			//текущий символ ленты == вершине стека, продвинулась лента, рор стека
			TS_NOK,
			//текущий символ ленты != вершине стека, восстановлено состояние
			LENTA_END,
			//текущая позиция ленты >= lenta_size
		...
		char* getCSt(char* buf) const; //получить содержимое стека
		char* getCLenta(char* buf, short pos, short n = 25) const; //лента: n символов с pos
		char* getDiagnosis(short n, char* buf) const; //получить n-ую строку диагностики или 0х00
